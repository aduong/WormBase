#!/usr/bin/perl
# t/WormBase/API/Benchmark/corocurrent1.pl
...
while () {
    ...
    while (keys %children < $WORKERS) {
        ...
        my $child = {
            reqpipe  => IO::Pipe->new,
            workpipe => IO::Pipe->new,
        };
        my $pid = fork;
        unless ($pid) { # child or failed fork
            die "Couldn't fork: $!" unless defined $pid;

            $child->{reqpipe}->writer;  # add reqs
            $child->{workpipe}->reader; # read work

            # finally run...
            child_run($child->{workpipe}, $child->{reqpipe});
            exit;
        }

        $child->{reqpipe}->reader;  # accept requests
        $child->{workpipe}->writer; # give work

        $select->add($child->{reqpipe});

        # we need the poll_reader key because select will return it
        $children{$pid} = $child;
        $poller2pid{$child->{reqpipe}} = $pid;
    }
...
sub reap {
    ...
    while ((my $pid = waitpid(-1, WNOHANG)) > 0) {
        my $poller = $children{$pid}->{reqpipe};
